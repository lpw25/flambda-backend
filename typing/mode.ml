...
module Lattices_mono = struct
  ...
  let src : type a b d. b obj -> (a, b, d) morph -> a obj =
   fun dst f ->
    match f with
  ...
  let equal_morph :
     ...
     fun dst f1 f2 ->
      ...
      | ((Simple _ | Proj_and _ | And_max_with _ | And_min_with _), _) ->
          Not_equal
  ...
  let print_locality_morph :
      type a b d. Format.formatter -> (a, b, d) locality_morph -> unit =
    fun ppf -> function
  ...
  let print_core_morph :
      type a b d. Format.formatter -> (a, b, d) core_morph -> unit =
    fun ppf -> function
    | Locality_restricted l -> print_locality_morph ppf l
    | Locality_full l ->
        Format.fprintf ppf "%a_full" print_locality_morph l
    | Uniqueness_op_to_linearity ->
        Format.fprintf ppf "uniqueness_op_to_linearity"
    | Linearity_to_uniqueness_op ->
        Format.fprintf ppf "linearity_to_uniqueness_op"
    | Contention_op_to_portability ->
        Format.fprintf ppf "contention_op_to_portability"
    | Portability_to_contention_op ->
        Format.fprintf ppf "portability_to_contention_op"
    | Visibility_op_to_statefulness ->
        Format.fprintf ppf "visibility_op_to_statefulness"
    | Statefulness_to_visibility_op ->
        Format.fprintf ppf "statefulnes_to_visibility_op"
    | Monadic_to_comonadic_min ->
        Format.fprintf ppf "monadic_to_comonadic_min"
    | Comonadic_to_monadic_min _ ->
        Format.fprintf ppf "comonadic_to_monadic_min"
    | Monadic_to_comonadic_max ->
        Format.fprintf ppf "monadic_to_comonadic_max"
    | Comonadic_to_monadic_max _ ->
        Format.fprintf ppf "comonadic_to_monadic_max"

  let rec print_simple_morph :
      type a b d. b obj -> Format.formatter -> (a, b, d) simple_morph -> unit =
   fun dst ppf -> function
    | Id -> Format.fprintf ppf "id"
    | Core m -> print_core_morph ppf m
    | Meet_const c -> Format.fprintf ppf "meet(%a)" (print dst) c
    | Imply_const c -> Format.fprintf ppf "imply(%a)" (print dst) c
    | Core_and_meet_const(c, m) ->
        Format.fprintf ppf "meet(%a) . %a"
          (print dst) c print_core_morph m
    | Imply_const_and_core(m, c) ->
        Format.fprintf ppf "%a . imply(%a)"
  ...
  let print_morph :
   ...
   fun dst ppf -> function
    ...
    | Proj_and(Id, ax, src) ->
        Format.fprintf ppf "proj_%a" print_obj (proj_obj ax src)
    | Proj_and(m, ax, src) ->
        Format.fprintf ppf "%a . proj_%a"
  ...
  let compose :
   ...
   fun _dst _m0 _m1 ->
    ...
    | Id, m -> m
    | m, Id -> m
    | _, And_max_with(m1, ax1) ->
        And_max_with(compose dst m0 m1, ax1)
    | _, And_min_with(m1, ax1) ->
        And_min_with(compose dst m0 m1, ax1)
    | Proj_and(obj0, ax0, m0), _ ->
        Proj_and(obj0, ax0, compose obj0 m0 m1)
    | And_max_with(m0, ax0), Proj_and(obj1, ax1, m1)-> begin
        let mid = src dst m0 in
        let c = min_with mid ax0 (max (proj_obj ax0 mid)) in
        let m0 = compose_imply_const_right dst m0 c in
        let m0 = connect_projections dst m0 (src dst m0) ax0 obj1 ax1 in
        compose dst m0 m1
      end
    | And_min_with(m0, ax0), Proj_and(obj1, ax1, m1)-> begin
        let mid = src dst m0 in
        let c = min_with mid ax0 (max (proj_obj ax0 mid)) in
  ...
  module For_hint = struct
    ...
    let find_responsible_axis_proj : type a b b_ax l r.
    ...
    let find_responsible_axis_all : type a b l r.
...
module Value_with (Areality : Areality) = struct
  ...
  module Const = struct
    ...
    let monadic_to_comonadic_min =
      C.monadic_to_comonadic_min Comonadic.Obj.obj
